...
/**
 * 
 * Command Handler for /rg setteleport
 * @author Jimi
 *
 */
public class RegionSetTeleportExecutor implements CommandExecutor {
		...
		if(src instanceof Player) {
			...
			if(RegionUtils.hasPendingRegion(player)) {
				if(args.hasAny("location")) {
					...
					if(region.isLocal()) {
						((LocalRegion)region).setTeleportLocation(RegionLocationUtils.fromLocation(player, location));
						MessageUtils.sendSuccessMessage(player, RegionText.REGION_TELEPORT_LOCATION_UPDATED.getValue());
	...
	private String getCommandUsage() {
		return "/rg setteleport <location>";
	}
